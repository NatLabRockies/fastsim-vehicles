/* @ts-self-types="./fastsim_schema.d.ts" */
 ...
 * @returns {string}
 */
export function build_download_url(id, extension, base_url) {
    let deferred5_0;
    let deferred5_1;
    try {
        ...
        var ptr2 = isLikeNone(base_url) ? 0 : passStringToWasm0(base_url, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        var len2 = WASM_VECTOR_LEN;
        const ret = wasm.build_download_url(ptr0, len0, ptr1, len1, ptr2, len2);
        var ptr4 = ret[0];
        var len4 = ret[1];
        if (ret[3]) {
            ptr4 = 0; len4 = 0;
 ...
 * @returns {any}
 */
export function parse_vehicles_jsonl(text) {
    const ptr0 = passStringToWasm0(text, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.parse_vehicles_jsonl(ptr0, len0);
    if (ret[2]) {
 ...
 * @returns {any}
 */
export function search_entries(entries_json, query_json) {
    ...
    const len1 = WASM_VECTOR_LEN;
    const ret = wasm.search_entries(ptr0, len0, ptr1, len1);
    if (ret[2]) {
        throw takeFromExternrefTable0(ret[1]);
    }
    return takeFromExternrefTable0(ret[0]);
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('fastsim_schema_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
